...
default_asset_version=${default_asset_version:=${DEFAULT_ASSET_VERSION:=1.19.3}}
...
then 
  ...
  for diagnostics_file in plugin_context.json gui_map.json rank_map.json particle_map.json entity_map.json; do
    if [[ -f "${diagnostics_file}" ]]; then
      cp -f "${diagnostics_file}" "./target/reports/${diagnostics_file}"
    fi
  done

  if [[ -f "./target/font_map.json" ]]; then
    cp -f "./target/font_map.json" "./target/reports/font_map.json"
  fi
