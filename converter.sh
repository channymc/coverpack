...
download_default_assets () {
  local requested_version="${1:-1.21.5}"
  local normalized_version
  local candidate
  local -a candidates=()

  normalized_version="$(printf '%s' "${requested_version}" | tr '[:upper:]' '[:lower:]')"
  ...
  add_candidate "${requested_version}"
  add_candidate "${normalized_version}"

  if [[ "${normalized_version}" =~ ^([0-9]+\.[0-9]+)\.[0-9]+$ ]]; then
    add_candidate "${BASH_REMATCH[1]}"
  fi

  if [[ "${normalized_version}" =~ ^1\.21(\.[0-9]+)?$ ]]; then
    add_candidate "1.21.5"
    add_candidate "1.21.4"
    add_candidate "1.21.3"
    add_candidate "1.21.2"
    add_candidate "1.21.1"
    add_candidate "1.21"
  fi

  add_candidate "1.20.6"
  add_candidate "1.20.4"
  add_candidate "1.20.2"
  add_candidate "1.20.1"
  add_candidate "1.19.4"
  add_candidate "1.19.3"

  for candidate in "${candidates[@]}"; do
    status_message process "Trying fallback resource pack version ${candidate}"
    printf "\e[3m\e[37m"
    echo
    rm -f default_assets.zip
    if COLUMNS=$COLUMNS-1 curl --no-styled-output -#fL -o default_assets.zip "https://github.com/InventivetalentDev/minecraft-assets/zipball/refs/tags/${candidate}"; then
      echo
      printf "${C_CLOSE}"
      default_asset_version="${candidate}"
      status_message completion "Fallback resources downloaded (${candidate})"
      return 0
    fi
    echo
    printf "${C_CLOSE}"
  ...
  status_message error "Unable to download fallback resources for requested version '${requested_version}'"
  return 1
}
...
ensure_geyser_mapping_from_script () {
  ...
  if [[ -f "script.json" ]]; then
    sprite_mapping_file="script.json"
  elif [[ -f "sprites.json" ]]; then
    sprite_mapping_file="sprites.json"
  fi

  if [[ -z "${sprite_mapping_file}" ]]; then
    return
  fi

  mkdir -p ./scratch_files
  if [[ ! -f ./target/geyser_mappings.json ]]; then
    jq -n '{"format_version":"1","items":{}}' | sponge ./target/geyser_mappings.json
  fi

  status_message process "Creating Geyser mappings from ${sprite_mapping_file}"
  jq -r '
  to_entries
  | map(.key as $item | (.value // []) | (if type == "array" then . else [] end) | map(select(type == "object") + {"item": $item}))
  | add[]?
  | [((.item | split(":")[-1]) + "_c" + (.custom_model_data | tostring) + "_d" + (.damage_predicate | tostring) + "_u" + (.unbreakable | tostring)), .sprite, .item, (.custom_model_data | tostring), (.damage_predicate | tostring), (.unbreakable | tostring)]
  | @tsv
  ' "${sprite_mapping_file}" > scratch_files/sprites.tsv

  if [[ ! -s scratch_files/sprites.tsv ]]; then
    status_message critical "Sprite mapping file was provided but contains no usable entries"
    return
  fi

  write_sprite_hash_entry () {
    local predicate="${1}"
    local icon="${2}"
    local item_identifier="${3}"
    local custom_model_data="${4}"
    local damage_predicate="${5}"
    local unbreakable="${6}"
    local output_file="${7}"
    local entry_hash

    entry_hash=$(echo -n "${predicate}" | md5sum | head -c 7)
    printf "%s\t%s\t%s\t%s\t%s\t%s\n" "${item_identifier}" "${icon}" "gmdl_${entry_hash}" "${custom_model_data}" "${damage_predicate}" "${unbreakable}" >> "${output_file}"
  }

  : > scratch_files/sprite_hashes.tsv
  while IFS=$'\t' read -r predicate icon item_identifier custom_model_data damage_predicate unbreakable
  do
    write_sprite_hash_entry "${predicate}" "${icon}" "${item_identifier}" "${custom_model_data}" "${damage_predicate}" "${unbreakable}" "scratch_files/sprite_hashes.tsv"
  done < scratch_files/sprites.tsv > /dev/null

  jq -cR 'split("\\t")' scratch_files/sprite_hashes.tsv | jq -s 'map({(.[2]): {"textures": .[1]}}) | add' > scratch_files/sprite_hashmap.json
  jq -cR 'split("\\t")' scratch_files/sprite_hashes.tsv | jq -s 'map({(.[2]): .[1]}) | add' > scratch_files/sprite_hash_lookup.json

  jq -cR 'split("\\t")' scratch_files/sprite_hashes.tsv | jq -s '
  map({
    "item": (if (.[0] | contains(":")) then .[0] else ("minecraft:" + .[0]) end),
    "mapping":
      ({
        "name": .[2],
        "allow_offhand": true,
      ...
      + (if .[3] != "null" and .[3] != "" then {"custom_model_data": (.[3] | tonumber)} else {} end)
      + (if .[4] != "null" and .[4] != "" then {"damage_predicate": (.[4] | tonumber)} else {} end)
      + (if ((.[5] | ascii_downcase) == "true") then {"unbreakable": true} else {} end))
  })
  ' > scratch_files/sprite_mapping_entries.json
  ...
  | .[1] as $extra_entries
  | .[2] as $base_mappings
  | ($base_mappings.items // {}) as $base_items
  | ($extra_entries | sort_by(.item) | group_by(.item) | map({(.[0].item): map(.mapping)}) | add // {}) as $extra_items
  | {
      "format_version": ($base_mappings.format_version // "1"),
      "items": (
        ...
        | map({
            ...
            "value": (
              ((($base_items[.key] // [])
                | map(.icon as $icon | .icon = ($icon_lookup[$icon] // $icon)))
              + ($extra_items[.key] // []))
              | unique_by(.name, .custom_model_data, .damage_predicate, .unbreakable)
            )
          })
        | from_entries
      )
    }
  ' scratch_files/sprite_hash_lookup.json scratch_files/sprite_mapping_entries.json ./target/geyser_mappings.json | sponge ./target/geyser_mappings.json

  status_message completion "Geyser mappings generated from ${sprite_mapping_file}"
}

collect_conversion_diagnostics () {
  status_message process "Collecting conversion diagnostics"
  mkdir -p ./target/reports

  for diagnostics_file in plugin_context.json gui_map.json rank_map.json particle_map.json entity_map.json block_map.json coverage_report.json; do
    if [[ -f "${diagnostics_file}" ]]; then
      cp -f "${diagnostics_file}" "./target/reports/${diagnostics_file}"
    fi
  done

  for diagnostics_file in ./target/font_map.json ./target/coverage_report.json ./target/geyser_mappings.json ./target/rp/sounds/sound_definitions.json ./target/rp/sounds/sound_diagnostics.json; do
    if [[ -f "${diagnostics_file}" ]]; then
      cp -f "${diagnostics_file}" "./target/reports/$(basename "${diagnostics_file}")"
    fi
  done
}
...
# resolve helper scripts regardless of whether converter.sh is run from root or staging
resolve_aux_script () {
  local script_name="${1}"
  local script_dir
...
auto_detect_input_pack () {
  local preferred="input_pack.zip"
  if [[ -f "${preferred}" ]]; then
    printf "%s" "${preferred}"
    return
  fi
  ...
  local detected
  detected="$(find . -maxdepth 1 -type f \( -iname '*.zip' -o -iname '*.jar' -o -iname '*.mcpack' \) | sed 's#^\./##' | sort | head -n 1)"
  printf "%s" "${detected}"
}
...
download_pack () {
  ...
  status_message process "Downloading input pack from URL"
  printf "\e[3m\e[37m"
  echo
  COLUMNS=$COLUMNS-1 curl --no-styled-output -#L -o "${output_file}" "${source_url}"
  echo
  printf "${C_CLOSE}"
  status_message completion "Input pack downloaded to ${output_file}"
}
...
resolve_input_pack () {
  ...
  if [[ -z "${resolved}" ]]; then
    if [[ -n "${PACK_URL}" ]]; then
      resolved="input_pack.zip"
      download_pack "${PACK_URL}" "${resolved}"
    else
      resolved="$(auto_detect_input_pack)"
    fi
  elif is_url "${resolved}"; then
    local source_url="${resolved}"
    resolved="input_pack.zip"
    download_pack "${source_url}" "${resolved}"
  fi
...
run_manager_hooks () {
  local input_pack_path="${1}"
  local resolved_input_pack
  local manager_script
  manager_script="$(resolve_aux_script manager.py)"

  if [[ "${input_pack_path}" = /* ]] || [[ "${input_pack_path}" =~ ^[A-Za-z]:[\\/] ]]; then
    resolved_input_pack="${input_pack_path}"
  else
    resolved_input_pack="$(pwd)/${input_pack_path}"
  fi

  if [[ -n "${manager_script}" ]]; then
    status_message process "Running post-processing manager hooks"
    if ! INPUT_PACK_PATH="${resolved_input_pack}" python "${manager_script}"; then
      status_message critical "manager.py post-processing failed; continuing packaging"
    fi
  else
    status_message critical "manager.py not found near converter.sh; skipping post-processing hooks"
  fi
}

# resolve input pack path
input_pack="$(resolve_input_pack "${1}")"

if ! test -f "${input_pack}"; then
   status_message error "Input resource pack not found. Provide it as first argument or place a single .zip/.jar/.mcpack beside converter.sh"
   exit 1
else
  status_message process "Input file ${input_pack} detected"
fi

# normalize positional arguments so the rest of the script can keep using ${1}
set -- "${input_pack}" "${@:2}"
...
merge_input=${merge_input:=${MERGE_INPUT:=null}}
attachable_material=${attachable_material:=${ATTACHABLE_MATERIAL:=entity_alphatest_one_sided}}
block_material=${block_material:=${BLOCK_MATERIAL:=alpha_test}}
fallback_pack=${fallback_pack:=${FALLBACK_PACK:=null}}
default_asset_version=${default_asset_version:=${DEFAULT_ASSET_VERSION:=1.21.5}}
rename_model_files=${rename_model_files:=${RENAME_MODEL_FILES:=false}}
save_scratch=${save_scratch:=${SAVE_SCRATCH:=false}}
disable_ulimit=${disable_ulimit:=${DISABLE_ULIMIT:=false}}
...
if [[ ${warn} != "false" ]]; then
  status_message info "Zero-touch mode keeps prompts disabled; continuing without confirmation prompt."
fi
...
# automatically run sg.py to generate script/sprites mappings with zero manual steps
sg_script="$(resolve_aux_script sg.py)"
if [[ -n "${sg_script}" ]]; then
  status_message process "Running sg.py for automatic script mapping generation"
  if SG_OUTPUT_DIR="$(pwd)" python "${sg_script}" "${1}"; then
    if [[ -f "script.json" && ! -f "sprites.json" ]]; then
      cp "script.json" "sprites.json"
    elif [[ -f "sprites.json" && ! -f "script.json" ]]; then
      cp "sprites.json" "script.json"
    fi
    status_message completion "sg.py mapping generation complete"
  else
    status_message critical "sg.py failed; conversion will continue without generated script mapping"
  fi
else
...
then 
  ...
  if [[ -f "script.json" ]]; then
    cp -f "script.json" "./target/script.json"
  fi
  if [[ -f "sprites.json" ]]; then
    cp -f "sprites.json" "./target/sprites.json"
  fi
...
then
  ...
  if ! download_default_assets "${default_asset_version:=1.21.5}"; then
    exit 1
  fi
fi
...
if test -f ${merge_input}; then
  ...
  if test -f ./inputbedrockpack/texts/languages.json; then
    ...
    jq -s '.[0] + .[1] | unique' ./target/rp/texts/languages.json ./inputbedrockpack/texts/languages.json | sponge ./target/rp/texts/languages.json
  fi
...
# Add sprites/script mapping if script.json or sprites.json exists in the root pack
sprite_mapping_file=""
if [ -f script.json ]; then
  sprite_mapping_file="script.json"
elif [ -f sprites.json ]; then
...
if [[ -n "${sprite_mapping_file}" ]]; then
  status_message process "Adding provided sprite paths from ${sprite_mapping_file}"
  jq -r '
  to_entries 
  | map(.key as $item | (.value // []) | (if type == "array" then . else [] end) | map(select(type == "object") + {"item": $item})) 
  | add[]? 
  ...
  if [[ ! -s scratch_files/sprites.tsv ]]; then
    ...
    while IFS=$'\t' read -r predicate icon item_identifier custom_model_data damage_predicate unbreakable
    do
      write_id_hash "${predicate}" "${icon}" "${item_identifier}" "${custom_model_data}" "${damage_predicate}" "${unbreakable}" "scratch_files/sprite_hashes.tsv"
    done < scratch_files/sprites.tsv > /dev/null
