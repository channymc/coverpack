...
def java_tex_to_sprite(tex_ref: str) -> str:
    ...
    For minecraft namespace: uses BASENAME ONLY (flat) to match converter.sh.
    For custom namespaces: preserves full subdirectory path so each texture
    gets a unique sprite path (e.g. skins/default/skin stays distinct from
    skins/alex/skin).
    ...
    if bedrock:
        ...
        # Custom namespace with a deep path → preserve full subpath after folder
        if ns != 'minecraft' and len(parts) > 2:
            rel = '/'.join(parts[1:])
            return f'{bedrock}/{rel}'
        return f'{bedrock}/{base}'
    # Unknown folder — preserve full path for custom namespaces
    if ns != 'minecraft' and len(parts) > 1:
        return f'textures/{"/".join(parts)}'
            ...
            for ns, fpath in new_files:
                ...
                if added == 0:
                    ...
                    if isinstance(base_model, str) and base_model.strip():
                        ...
                        sprite, fallback = resolve_sprite(clean, stem, res, reader, ns or m_ns)

                    if not sprite:
                        sprite = _resolve_sprite_from_node(mn, stem, res, reader, ns)
                        fallback = False
