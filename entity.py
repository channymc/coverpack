...
EXTRA_TEXTURE_CATEGORIES = (
    ("painting", "assets/**/textures/painting/**/*.png", "assets/**/textures/painting/*.png"),
    ("particle", "assets/**/textures/particle/**/*.png", "assets/**/textures/particle/*.png"),
    ("painting", "assets/**/textures/painting/**/*.tga", "assets/**/textures/painting/*.tga"),
    ("particle", "assets/**/textures/particle/**/*.tga", "assets/**/textures/particle/*.tga"),
)
...
def _collect_extra_textures() -> Dict[str, List[str]]:
    ...
    for category, *patterns in EXTRA_TEXTURE_CATEGORIES:
        for pattern in patterns:
            for tex_file in PACK_DIR.glob(pattern):
                if not tex_file.is_file() or tex_file in seen:
                    continue
                seen.add(tex_file)
                rel = tex_file.relative_to(PACK_DIR).as_posix()
                parts = rel.split("/")
                try:
                    tex_idx = parts.index("textures")
                except ValueError:
                    continue
                output_rel = "/".join(parts[tex_idx:])
                destination = TARGET_RP_DIR / output_rel
                destination.parent.mkdir(parents=True, exist_ok=True)
                shutil.copyfile(tex_file, destination)
                mcmeta = tex_file.with_suffix(tex_file.suffix + ".mcmeta")
                if mcmeta.exists():
                    shutil.copyfile(mcmeta, destination.with_suffix(destination.suffix + ".mcmeta"))
                result.setdefault(category, []).append(output_rel)
    for key in result:
