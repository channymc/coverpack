...
TARGET_ITEM_DIR = Path("staging/target/rp/textures/items")
...
def _copy_item_texture(texture: Path) -> Optional[Dict[str, Any]]:
    """Copy a custom-namespace item texture to textures/items/<subpath> preserving full relative path."""
    parts = texture.as_posix().split("/")
    try:
        assets_index = parts.index("assets")
        textures_index = parts.index("textures", assets_index)
        folder = parts[textures_index + 1] if textures_index + 1 < len(parts) else ""
        if folder != "item":
            return None
        rel = "/".join(parts[textures_index + 2 :])
    except Exception:
        return None
    if not rel:
        return None

    destination = TARGET_ITEM_DIR / rel
...
def _collect_item_textures() -> List[Dict[str, Any]]:
    """Copy ALL item textures from non-minecraft namespaces to textures/items/."""
    copied: List[Dict[str, Any]] = []
    seen: Set[Path] = set()
    TARGET_ITEM_DIR.mkdir(parents=True, exist_ok=True)

    for texture in PACK_DIR.glob("assets/**/textures/item/**/*"):
        if not texture.is_file() or texture in seen:
            continue
        if texture.suffix.lower() not in (".png", ".tga"):
            continue
        rel = texture.relative_to(PACK_DIR).as_posix()
        if rel.startswith("assets/minecraft/"):
            continue
        seen.add(texture)
        meta = _copy_item_texture(texture)
        if meta:
            copied.append(meta)
...
def _count_direct_item_models() -> int:
    """Count new-format items/*.json that have a direct model ref with no CMD predicate."""
    count = 0
    seen: Set[Path] = set()
    for pattern in ("assets/**/items/*.json", "assets/**/items/**/*.json"):
        for item_file in PACK_DIR.glob(pattern):
            ...
            if not isinstance(mn, dict):
                continue
            node_type = str(mn.get("type", "")).strip().lower()
            if node_type in {
                ...
                has_cmd = bool(_extract_cmd_values(data))
                if not has_cmd:
                    count += 1
    return count
...
def run() -> None:
    ...
    item_textures = _collect_item_textures()
    model_mappings = _collect_model_mappings()
    direct_item_count = _count_direct_item_models()
    ...
    payload = {
        ...
        "direct_item_model_count": direct_item_count,
        "texture_count": len(textures) + len(item_textures),
        "item_texture_count": len(item_textures),
        ...
        "textures": textures + item_textures,
    ...
    _log(
        ...
        f"{len(textures)} GUI textures, {len(item_textures)} item textures"
    )
